...
def main(args):
    ...
    for ino in range(len(img_list)):
        filename = valid_image_file_list[ino].split('\\')[-1]
        with open('C:/Studies/Practice/Чистый2.csv', 'r') as file1:
            for line in file1:
                # print(line)
                # print(file_name+',')
                if filename in line:
        ...
        #                                       rec_res[ino]))
        print(f'File: {valid_image_file_list[ino]}')
        print(f'Answer: "{rec_res[ino][0]}"')
    print("Удачно: ", c)
    if args.benchmark:
